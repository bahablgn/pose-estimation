...
class TfPoseEstimator:
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            ...
            rightZeroFlag = False
            for _k in range(22, 28):
                if (int(flat[_k]) != 0):
                    print('rightLoop', _k, int(flat[_k]))
                    continue
                else:
                    print('rightLoop', _k, int(flat[_k]))
                    rightZeroFlag = True
            ...
            if (leftZeroFlag == False and rightZeroFlag):
                print('leftZeroFlag | rightZeroFlag',leftZeroFlag,rightZeroFlag)
                # ratio1
                point1 = [flat[16], flat[17]]
                point2 = [flat[18], flat[19]]

                point3 = [flat[20], flat[21]]
                point4 = [flat[18], flat[19]]
                print(point1, point2)
                print(point3, point4)
                dist1 = euclidian(point1, point2)
                dist2 = euclidian(point3, point4)
                print('Rhip-Rknee', dist1)
                print('Rknee-Rankle', dist2)
                ratio1 = dist1 / dist2
                print(ratio1)

                a = math.sqrt((flat[18] - flat[16]) ** 2 + (flat[19] - flat[17]) ** 2)
                b = math.sqrt((flat[18] - flat[20]) ** 2 + (flat[19] - flat[21]) ** 2)
                c = math.sqrt((flat[20] - flat[16]) ** 2 + (flat[21] - flat[17]) ** 2)
                angle = math.acos(((a ** 2) + (b ** 2) - (c ** 2)) / (2 * a * b)) * 180 / math.pi
                print(angle)
                ind = ratio1 + (angle / 360)

                if (ind < thereshold):
                    label = 'Sitting'
                else:
                    label = 'Standing'
            elif (leftZeroFlag and rightZeroFlag == False):
                print('leftZeroFlag | rightZeroFlag',leftZeroFlag,rightZeroFlag)

                # ratio1
                point1 = [flat[22], flat[23]]
                point2 = [flat[24], flat[25]]

                point3 = [flat[26], flat[27]]
                point4 = [flat[24], flat[25]]
                print(point1, point2)
                dist1 = euclidian(point1, point2)
                dist2 = euclidian(point3, point4)
                print('Lhip-Lknee', dist1)
                print('Lknee-Lankle', dist2)
                ratio1 = dist1 / dist2
                print(ratio1)

                a = math.sqrt((flat[24] - flat[22]) ** 2 + (flat[25] - flat[23]) ** 2)
                b = math.sqrt((flat[24] - flat[26]) ** 2 + (flat[25] - flat[27]) ** 2)
                c = math.sqrt((flat[26] - flat[22]) ** 2 + (flat[27] - flat[23]) ** 2)
                angle = math.acos(((a ** 2) + (b ** 2) - (c ** 2)) / (2 * a * b)) * 180 / math.pi
                print(angle)
                ind = ratio1 + (angle / 360)
                ...
                if (ind < thereshold):
                    ...
                    label = 'Standing'
            elif (leftZeroFlag == False and rightZeroFlag == False):
                print('leftZeroFlag | rightZeroFlag', leftZeroFlag, rightZeroFlag)
                # ratio1
                point1 = [flat[16], flat[17]]
                point2 = [flat[18], flat[19]]
            ...
            # if(controlFlag):
            #     # ratio2
            #     point1=[flat[0],flat[1]]
            #     point2=[flat[20],flat[21]]
            #
            #     point3=[flat[16],flat[17]]
            #     point4=[flat[18],flat[19]]
            #     # ratio1
            #     point1=[flat[16],flat[17]]
            #     pointNose=[flat[16],flat[17]]
            #     point2=[flat[18],flat[19]]
            #
            #     point3=[flat[20],flat[21]]
            #     point4=[flat[18],flat[19]]
            ...
            # bottomLeftCornerOfText = ([flat[0], flat[1]])
            bottomLeftCornerOfText = (flat[0]-70, flat[1]-70)
            fontScale = 1

            fontColor = (255, 0, 0)
            # cv2.putText(npimg,label,
            #             (10, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
            #             (0, 0, 255), 2)

            TEXT_FACE = cv2.FONT_HERSHEY_DUPLEX
            TEXT_SCALE = 1
            scale = 0.050  # this value can be from 0 to 1 (0,1] to change the size of the text relative to the image
            fontScale = min(image_w, image_h) / (25 / scale)
            TEXT_THICKNESS = 1
            TEXT = label
            text_size, _ = cv2.getTextSize(TEXT, TEXT_FACE, fontScale, TEXT_THICKNESS)
            text_y = int(flat[1])
            text_y_subs = image_h - text_y
            text_y_position = int(text_y_subs + (text_y / 2))
            text_y_half = int(text_y/2)
            #text_y_posit = text_y - text_y_half
            text_y_posit = text_y - ((image_h*9) / 100)

            print(image_h)
            print(text_y)
            print(text_y_subs)
            print(text_y_position)
            text_origin = (int(flat[0] - text_size[0] / 2), int(text_y_posit))
            cv2.putText(npimg, TEXT, text_origin, TEXT_FACE, fontScale, (0, 255, 0), TEXT_THICKNESS, cv2.LINE_AA)

            #cv2.putText(npimg, label, bottomLeftCornerOfText, font, fontScale, fontColor, 1)
